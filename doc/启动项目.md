...
| neo4j + pyahocorasick + python-dotenv + elasticsearch | 必需 | 见 `requirements.txt` |
...
**方式二：Docker（Mac Studio M4 推荐）**

详见 **[Neo4j-Docker部署.md](./Neo4j-Docker部署.md)**，快速启动：
...
## 4.5 启动 Elasticsearch 并建立检索索引（推荐）

实体召回与 RAG 文档检索依赖 Elasticsearch。在项目目录执行：
...
ES_ENABLED=true
ENTITY_LINK_MIN_SCORE=3.0
```

> ES 未启动时系统自动回退到 Aho-Corasick 词典匹配，结构化问答仍可用。

验证实体召回（可选）：

```bash
python scripts/eval_entity_recall.py
python scripts/eval_routing.py          # 口语查病 → RAG 路由
```
...
1. 加载 `dict/` 词典，构建 Aho-Corasick 自动机（ES 可用时叠加实体链接）
...
| 实体识别不准 / 错别字无法识别 | ES 未索引 | `docker compose up -d` 后运行 `python -m retrieval.indexer` |
| 开放问句无 LLM 回答 | 未配置 API Key | 在 `.env` 设置 `LLM_API_KEY` 等变量 |
...
| [doc/Neo4j-Docker部署.md](./Neo4j-Docker部署.md) | Mac M4 Docker 部署 Neo4j |
